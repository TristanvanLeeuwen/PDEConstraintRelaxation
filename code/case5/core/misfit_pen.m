...
for k = 1:length(model.f) 
    ...
    A = [sqrt(lambda)*At;Pr]'*[sqrt(lambda)*At;Pr];
    b = [sqrt(lambda)*At;Pr]'* [sqrt(lambda)*Ps'*Q;reshape(D,nr,ns)];
	Ut = A\b;

	% compute misfit, gradient and Hessian
    f = .5*lambda*norm(At*Ut - Ps'*Q,'fro').^2 +...
            .5*norm(Pr*Ut - reshape(D,nr,ns),'fro').^2;
